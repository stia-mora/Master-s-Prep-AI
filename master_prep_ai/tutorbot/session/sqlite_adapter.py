"""SessionManager adapter backed by Master Prep AI's SQLite store.

Implements the SessionManager interface (get_or_create, save, list_sessions) but
reads/writes through Master Prep AI's SQLiteSessionStore, unifying conversation history
for TutorBot and Master Prep AI in a single database.
"""
...
from master_prep_ai.tutorbot.session.manager import Session


class SQLiteSessionAdapter:
    """Drop-in replacement for SessionManager, backed by Master Prep AI SQLite."""

    def __init__(self, store: Any) -> None:
        """
        Args:
            store: A Master Prep AI SQLiteSessionStore instance.
        """
        self.store = store
    ...
    def _session_id(self, key: str) -> str:
        """Derive a stable Master Prep AI session_id from a TutorBot key (channel:chat_id)."""
    ...
    def _ensure_sqlite_session_sync(self, key: str) -> None:
        """Ensure a corresponding Master Prep AI session row exists."""
        session_id = self._session_id(key)
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
