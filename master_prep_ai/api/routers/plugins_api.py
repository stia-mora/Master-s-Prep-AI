...
from master_prep_ai.logging import ConsoleFormatter
...
def _discover_plugins() -> list[Any]:
    try:
        from master_prep_ai.plugins.loader import discover_plugins
    except Exception:
...
def _collect_project_loggers() -> list[logging.Logger]:
    ...
    for parent_name in ("master_prep_ai", "src"):
    ...
    for name, logger_obj in logging.root.manager.loggerDict.items():
        if not (name.startswith("master_prep_ai") or name.startswith("src")):
            continue
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    """Run a tool while capturing all master_prep_ai.* logs and yielding SSE events."""
    registry = get_tool_registry()
    tool = registry.get(tool_name)
    if not tool:
