...
from master_prep_ai.config.constants import PROJECT_ROOT
...
class Logger:
    """
    Unified logger for Master Prep AI.
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(f"master_prep_ai.{name}")
        ...
        if file_output:
            ...
            log_file = log_dir_path / f"master_prep_ai_{timestamp}.log"
...
def get_logger(
    ...
    if log_dir is None or effective_level is None:
        try:
            from master_prep_ai.services.config import get_path_from_config, load_config_with_main
