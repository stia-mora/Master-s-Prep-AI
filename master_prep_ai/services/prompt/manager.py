...
from master_prep_ai.services.config import PROJECT_ROOT, parse_language
...
class PromptManager:
    ...
    # Modules that are not under master_prep_ai/agents/ directory
    # Map module_name → on-disk path component under master_prep_ai/
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PROJECT_ROOT / "master_prep_ai" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PROJECT_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PROJECT_ROOT / "master_prep_ai" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]
