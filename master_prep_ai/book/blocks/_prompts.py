"""
Shared prompt loader for non-BaseAgent call sites in ``master_prep_ai/book``.

All LLM-facing prompts inside this module live as YAML under
``master_prep_ai/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~master_prep_ai.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``master_prep_ai/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected master_prep_ai/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
