...
<<<<<<< HEAD
            ai_meta={"message": "Practice questions were generated by filters.", "session_type": session_type, "task_id": task_id},
            user_id=user_id,
        )
...
<<<<<<< HEAD
        ...
        # Capture mastery before
        k_ids = {q["knowledge_id"] for q in questions if q.get("knowledge_id")}
        mastery_before = {kid: self.learning_store.get_mastery_score(kid, user_id) for kid in k_ids}
...
<<<<<<< HEAD
        ...
        # Capture mastery after and calculate delta
        mastery_after = {kid: self.learning_store.get_mastery_score(kid, user_id) for kid in k_ids}
        mastery_delta = {kid: mastery_after[kid] - mastery_before.get(kid, 50.0) for kid in mastery_after}

        record["answers"] = results
        record["answer_records"] = results
        record["score"] = int(record["accuracy"] * 100)
...
>>>>>>> 119a19f1a4d8666491536297b869396cbe7efd83
    ...
    async def _grade_free_response(self, question: dict[str, Any], user_answer: str, image_data_url: str, user_id: str) -> dict[str, Any]:
        ...
        async def mock_math_eval(q, a, img):
            # Placeholder for Member D's specialized math evaluator
            return None # Fallback to LLM if None
