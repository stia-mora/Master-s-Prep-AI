...
def _matches_question_family(question: dict[str, Any], question_family: str | None) -> bool:
    if question_family not in {"choice", "free_response"}:
        return True
    is_choice = bool(question.get("is_choice"))
    return is_choice if question_family == "choice" else not is_choice
...
class KaoyanContentStore:
    ...
    def select_questions(
        ...
        *,
        ...
        fetch_limit = max(1, min(limit, 30))
        params.append(fetch_limit * 4 if question_family else fetch_limit)
        with self._connect() as conn:
            rows = conn.execute(
                f"""
                SELECT question_id, knowledge_id, question_type, difficulty_level, stem,
        ...
        questions = _question_rows(rows)
        if question_family:
            questions = [item for item in questions if _matches_question_family(item, question_family)]
        return questions[:fetch_limit]
