...
class KaoyanLearningStore:
    ...
    def _initialize(self) -> None:
        with self._connect() as conn:
            ...
            # PR-B: wrong_question extended columns
            self._ensure_column(conn, "wrong_question", "wrong_reason", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(conn, "wrong_question", "retry_count", "INTEGER NOT NULL DEFAULT 0")
            self._ensure_column(conn, "wrong_question", "last_retry_mode", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(conn, "wrong_question", "priority_score", "REAL NOT NULL DEFAULT 5.0")
            conn.commit()
    ...
    # ---------------------------------------------------------------------- #
    #  PR-B: Wrong question summary                                            #
    # ---------------------------------------------------------------------- #

    def wrong_questions_summary(self, user_id: str = DEFAULT_USER_ID) -> dict[str, Any]:
        """Return aggregated statistics for wrong questions (PR-B)."""
        with self._connect() as conn:
            # Per knowledge_id groups
            ...
            reason_rows = conn.execute(
                """
                SELECT COALESCE(NULLIF(wrong_reason,''), NULLIF(error_reason,''), '未分类') AS reason,
                       COUNT(*) AS count
                FROM wrong_question
                WHERE user_id = ? AND review_status != 'mastered'
                GROUP BY reason
            ...
            # Trend: wrong questions added in last 7 days
            trend_rows = conn.execute(
                """
                SELECT DATE(last_wrong_at) AS day, COUNT(*) AS count
                FROM wrong_question
                WHERE user_id = ? AND last_wrong_at >= DATE('now', '-7 days')
                GROUP BY day
                ORDER BY day
                """,
                (user_id,),
            ...
            blocked_count = conn.execute(
                "SELECT COUNT(*) FROM wrong_question WHERE user_id = ? AND review_status != 'mastered' AND wrong_count >= 3",
                (user_id,),
            ).fetchone()[0]

            total_pending = conn.execute(
                "SELECT COUNT(*) FROM wrong_question WHERE user_id = ? AND review_status != 'mastered'",
                (user_id,),
            ).fetchone()[0]
        ...
        stage_groups = [
            {
                ...
                "wrong_count": int(row["count"]),
                "total_wrong": int(row["total_wrong"] or 0),
                "last_wrong_at": row["last_wrong_at"] or "",
                "avg_priority": float(row["avg_priority"] or 5.0),
            }
        ...
        return {
            ...
            "wrong_reason_distribution": reason_dist,
            "recent_wrong_trend": trend,
            "blocked_stage_count": int(blocked_count),
            "total_pending_wrong": int(total_pending),
        }

    # ---------------------------------------------------------------------- #
    #  PR-B: Record wrong retry result                                         #
    # ---------------------------------------------------------------------- #

    def record_wrong_retry(
        self,
        *,
        question_id: str,
        is_correct: bool,
        ...
        """Update wrong_question retry tracking and mastery after a retry."""
        now = utc_now()
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM wrong_question WHERE user_id = ? AND question_id = ?",
                (user_id, question_id),
            ).fetchone()

            old_mastery_row = conn.execute(
                "SELECT mastery_score FROM mastery_record WHERE user_id = ? AND knowledge_id = ?",
                (user_id, knowledge_id),
            ).fetchone()
            old_score = float(old_mastery_row["mastery_score"]) if old_mastery_row else 50.0

            if row is not None:
                new_priority = max(0.5, float(row["priority_score"] or 5.0) - (2.0 if is_correct else 0.0))
                new_status = "mastered" if is_correct else "pending"
                conn.execute(
                    """
                    UPDATE wrong_question
                    SET retry_count = retry_count + 1,
                        last_retry_mode = ?,
                        wrong_reason = CASE WHEN ? != '' THEN ? ELSE wrong_reason END,
                        priority_score = ?,
                        review_status = ?
                    WHERE user_id = ? AND question_id = ?
                    """,
                    (retry_mode, wrong_reason, wrong_reason, new_priority, new_status, user_id, question_id),
                )

            # Update mastery
            self._update_mastery(conn, knowledge_id, is_correct, user_id, now)

            # Compute score_delta for stage_attempt
            new_mastery_row = conn.execute(
                "SELECT mastery_score FROM mastery_record WHERE user_id = ? AND knowledge_id = ?",
                (user_id, knowledge_id),
            ).fetchone()
            new_score = float(new_mastery_row["mastery_score"]) if new_mastery_row else old_score
            ...
            conn.execute(
                ...
                INSERT INTO stage_attempt
                    (attempt_id, user_id, stage_id, session_id, source, score_delta, wrong_reason, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def add_stage_attempt(
        ...
        *,
        ...
        """Directly insert a stage_attempt record and return attempt_id."""
        now = utc_now()
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (attempt_id, user_id, stage_id, session_id, source, score_delta, wrong_reason, now),
            )
            conn.commit()
        return attempt_id

    def get_stage_progress(self, user_id: str = DEFAULT_USER_ID) -> list[dict[str, Any]]:
        """Return mastery_score + retry stats per knowledge_id (stage_progress view)."""
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT m.knowledge_id,
                       ...
                       COALESCE(w.last_retry_mode, '') AS last_retry_mode,
                       COALESCE(w.priority_score, 5.0) AS priority_score,
                       COALESCE(w.review_status, '') AS review_status
                FROM mastery_record m
                LEFT JOIN wrong_question w
                    ON m.user_id = w.user_id AND m.knowledge_id = w.knowledge_id
                WHERE m.user_id = ?
        ...
        return [
            {
                ...
                "last_practiced_at": row["last_practiced_at"] or "",
                "retry_count": int(row["retry_count"]),
                "last_retry_mode": row["last_retry_mode"] or "",
                "priority_score": float(row["priority_score"]),
                "review_status": row["review_status"] or "",
                "next_action": "继续刷题" if float(row["mastery_score"]) >= 75 else "重刷巩固",
            }
            for row in rows
        ]
