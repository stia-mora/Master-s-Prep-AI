...
RUN echo "NEXT_PUBLIC_API_BASE=http://master-prep-ai-api-placeholder.local" > .env.local
...
RUN pip install --upgrade pip -i https://pypi.tuna.tsinghua.edu.cn/simple && \
    pip install --retries 10 --timeout 120 -i https://pypi.tuna.tsinghua.edu.cn/simple -r requirements.txt
...
find /app/web/.next -type f \( -name "*.js" -o -name "*.json" \) -exec \
    sed -i "s|http://master-prep-ai-api-placeholder.local|${API_BASE}|g" {} \; 2>/dev/null || true
