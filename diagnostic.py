...
class KaoyanDiagnosticService:
    ...
    async def create_session(self, mode: str = "light", profile: dict[str, Any] | None = None, user_id: str = DEFAULT_USER_ID) -> dict[str, Any]:
        ...
        seed_questions = self.content_store.select_questions(question_family="choice", limit=min(limit, 8))
        ...
        questions = [item for item in questions if self._is_choice_question(item)]
        if len(questions) < limit:
            seen = {item["question_id"] for item in questions}
            questions.extend([item for item in seed_questions if item["question_id"] not in seen][: limit - len(questions)])
        if not questions:
    ...
    async def submit_session(self, session_id: str, answers: list[dict[str, Any]], user_id: str = DEFAULT_USER_ID) -> dict[str, Any] | None:
        ...
        profile_draft, ai_meta = await self._profile_draft(
            profile_seed, metadata.get("mode", "light"), results, user_id
        )
        return {
    ...
    async def _profile_draft(
        self, profile: dict[str, Any], mode: str, results: list[dict[str, Any]], user_id: str
    ...
    def _is_choice_question(self, question: dict[str, Any]) -> bool:
        return bool(question.get("is_choice")) or bool(question.get("options")) or "选择" in str(question.get("question_type") or "")
