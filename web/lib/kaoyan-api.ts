...
export function getProfile(): Promise<KaoyanProfile | null> {
  return request<KaoyanProfile | null>("/api/v1/kaoyan/profile/me");
}
...
export function createPracticePdf(input: PracticePdfRequest = {}): Promise<PracticePdfPayload> {
  return request<PracticePdfPayload>("/api/v1/kaoyan/practice/pdf", {
    method: "POST",
    body: JSON.stringify(input),
  });
}

export function downloadPracticePdf(input: PracticePdfRequest = {}): Promise<Blob> {
  return requestBlob("/api/v1/kaoyan/practice/pdf/download", {
    method: "POST",
    body: JSON.stringify(input),
  });
}
