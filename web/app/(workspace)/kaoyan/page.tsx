...
import { createDiagnosticSession, createKaoyanChatContext, createPracticePdf, createPracticeSession, downloadDailyReviewPdf, downloadPracticePdf as downloadPracticePdfBlob, generatePlan, getDashboardSummary, getDiagnosticReports, confirmDiagnosticReport, getKnowledgeDetail, getKnowledgeTree, getReviewsCalendar, getReviewsToday, getTodayTasks, getWrongQuestions, initProfile, startReviewTest, submitDiagnostic, submitPractice, submitReviewTest, updateTaskStatus } from "@/lib/kaoyan-api";
import type { ContentQuestion, DashboardSummary, DiagnosticReport, DiagnosticResult, KaoyanProfile, KnowledgeDetail, KnowledgeNode, PlanTask, PracticePdfPayload, PracticeResult, PracticeSession, ProfileDraft, ReviewCalendar, ReviewItem, ReviewTest, WrongQuestion } from "@/lib/kaoyan-types";
...
export default function KaoyanPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [tree, dashboard, today, wrong, reviewItems, calendar, reportItems] = await Promise.all([getKnowledgeTree(), getDashboardSummary(), getTodayTasks(), getWrongQuestions(), getReviewsToday(), getReviewsCalendar(reviewDate, reviewDate), reportsPromise]);
      setKnowledgeTree(tree); setSummary(dashboard); setTasks(today); setWrongQuestions(wrong); setReviews(reviewItems); setReviewCalendar(calendar); setDiagnosticReports(reportItems.reports || []);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[var(--background)]" translate="no">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-5">
        ...
        {activeTab === "review" ? <ReviewPanel reviews={reviews} calendar={reviewCalendar} selectedDate={reviewDate} activeTest={activeReviewTest} testAnswer={reviewTestAnswer} testResult={reviewTestResult} loading={loading} onDateChange={setReviewDate} onStart={handleStartReview} onAnswer={setReviewTestAnswer} onSubmitTest={handleSubmitActiveReviewTest} onDownloadPdf={handleDownloadReviewPdf} /> : null}
      </main>
    </div>
  );
}
...
function ReviewPanel({ reviews, calendar, selectedDate, activeTest, testAnswer, testResult, loading, onDateChange, onStart, onAnswer, onSubmitTest, onDownloadPdf }: { reviews: ReviewItem[]; calendar: ReviewCalendar | null; selectedDate: string; activeTest: ReviewTest | null; testAnswer: string; testResult: ReviewItem | null; loading: string | null; onDateChange: (date: string) => void; onStart: (review: ReviewItem) => void; onAnswer: (answer: string) => void; onSubmitTest: () => void; onDownloadPdf: (includeAnswers?: boolean) => void }) {
  const day = calendar?.days.find((item) => item.date === selectedDate);
  const queue = day?.items?.length ? day.items : reviews;
  const prompt = activeTest?.question?.stem_without_options || activeTest?.question?.stem || activeTest?.prompt || "";
  return <section className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">复习日历</h2><div className="text-xs text-[var(--muted-foreground)]">到期 {day?.due_count || queue.length} 项 · 逾期 {day?.overdue_count || 0} 项</div></div><div className="flex flex-wrap gap-2"><input type="date" value={selectedDate} onChange={(event) => onDateChange(event.target.value)} className="rounded-lg border bg-transparent px-3 py-2 text-sm" /><button onClick={() => onDownloadPdf(false)} disabled={loading === "review-pdf"} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm">{loading === "review-pdf" ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />} 学生版 PDF</button><button onClick={() => onDownloadPdf(true)} disabled={loading === "review-pdf-teacher"} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm">{loading === "review-pdf-teacher" ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />} 教师版 PDF</button></div></div>{calendar?.days?.length ? <div className="grid grid-cols-2 gap-2 md:grid-cols-7">{calendar.days.map((item) => <button key={item.date} type="button" onClick={() => onDateChange(item.date)} className={`rounded-lg border px-3 py-2 text-left text-xs ${item.date === selectedDate ? "border-[var(--primary)] bg-[var(--primary)] text-[var(--primary-foreground)]" : "hover:bg-[var(--muted)]"}`}><div className="font-medium">{item.date.slice(5)}</div><div className="mt-1 opacity-80">{item.due_count} 项</div></button>)}</div> : null}{activeTest ? <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"><div className="mb-2 text-sm font-semibold">{activeTest.title}</div><MarkdownRenderer content={prompt || "暂无题面"} variant="compact" />{activeTest.question?.options?.length ? <div className="mt-3 grid gap-2 md:grid-cols-2">{activeTest.question.options.map((option) => <button key={option.label} type="button" onClick={() => onAnswer(option.label)} className={`rounded-lg border px-3 py-2 text-left text-sm ${testAnswer === option.label ? "border-[var(--primary)] bg-[var(--primary)] text-[var(--primary-foreground)]" : "hover:bg-[var(--muted)]"}`}><span className="font-semibold">{option.label}. </span>{option.content}</button>)}</div> : <textarea className="mt-3 min-h-24 w-full rounded-lg border bg-transparent px-3 py-2 text-sm" placeholder="输入复检答案" value={testAnswer} onChange={(event) => onAnswer(event.target.value)} /> }<button onClick={onSubmitTest} disabled={loading === `review-submit-${activeTest.review_id}`} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-[var(--foreground)] px-4 py-2 text-sm text-[var(--background)]">{loading === `review-submit-${activeTest.review_id}` ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} 提交复检</button>{testResult ? <div className="mt-3 rounded-md border border-[var(--border)]/70 p-3 text-sm"><div className={testResult.is_correct ? "text-emerald-600" : "text-[var(--destructive)]"}>{testResult.is_correct ? "复检通过" : "复检未通过"} · 下次 {testResult.next_review_at ? new Date(testResult.next_review_at).toLocaleDateString() : "待安排"}{testResult.next_action ? ` · ${testResult.next_action}` : ""}</div>{testResult.result?.ai_analysis ? <MarkdownRenderer content={testResult.result.ai_analysis} variant="compact" /> : null}</div> : null}</div> : null}<div className="space-y-3"><h3 className="font-semibold">当天复习队列</h3>{queue.length === 0 ? <Empty text="暂无待复习项。错题和低掌握度知识点会自动进入队列。" /> : queue.map((review) => <div key={review.review_id} className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="font-medium">{review.title}</div><div className="text-xs text-[var(--muted-foreground)]">阶段 {review.stage_id || review.knowledge_id || "-"} · 间隔 {review.interval_days || 1} 天 · 已复习 {review.review_count} 次{review.next_action ? ` · ${review.next_action}` : ""}</div></div><button onClick={() => onStart(review)} className="rounded-md bg-[var(--foreground)] px-3 py-1.5 text-xs text-[var(--background)]" disabled={loading === `review-test-${review.review_id}`}>{loading === `review-test-${review.review_id}` ? "加载中" : "开始复检"}</button></div><div className="mt-3"><MarkdownRenderer content={review.prompt || "暂无题面"} variant="compact" /></div></div>)}</div></section>;
}
