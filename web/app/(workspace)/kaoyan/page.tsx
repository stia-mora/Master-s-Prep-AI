...
import { batchActionWrongQuestions, batchRetryWrongQuestions, createDiagnosticSession, createKaoyanChatContext, createPracticePdf, downloadPracticePdf as downloadPracticePdfBlob, explainLearningStageAgain, generatePlan, generatePractice, getDashboardSummary, getDiagnosticReports, getLearningPath, confirmDiagnosticReport, getKnowledgeDetail, getKnowledgeTree, getReviewsToday, getTodayTasks, getWrongQuestions, getWrongQuestionRecommendations, getWrongQuestionSummary, initProfile, refreshLearningPath, retryWrongQuestion, startLearningStage, submitDiagnostic, submitPractice, submitReview, updateTaskStatus, updateWrongQuestionReason } from "@/lib/kaoyan-api";
import type { ContentQuestion, DashboardSummary, DiagnosticReport, DiagnosticResult, ExplainAgainMode, ExplanationVariant, KaoyanProfile, KnowledgeDetail, KnowledgeNode, LearningPath, LearningStage, PlanTask, PracticePdfPayload, PracticeResult, PracticeSession, PracticeSource, ProfileDraft, QuestionKind, ReviewItem, WrongQuestion, WrongQuestionRecommendation, WrongQuestionRecommendationsResponse, WrongQuestionSummary } from "@/lib/kaoyan-types";
...
export default function KaoyanPage() {
  ...
  const [wrongRecommendations, setWrongRecommendations] = useState<WrongQuestionRecommendationsResponse | null>(null);
  ...
  const loadWrongData = useCallback(async () => {
    const wrongQuery = {
      status: wrongFilters.status || undefined,
      sort: wrongFilters.sort || undefined,
      question_type: wrongFilters.question_type || undefined,
      wrong_reason: wrongFilters.wrong_reason || undefined,
    };
    const [wrong, wrongStats, wrongRecs] = await Promise.all([
      getWrongQuestions(wrongQuery),
      getWrongQuestionSummary(),
      getWrongQuestionRecommendations(3).catch(() => null),
    ]);
    setWrongQuestions(wrong);
    setWrongSummary(wrongStats);
    setWrongRecommendations(wrongRecs);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const pathPromise = getLearningPath().catch(() => refreshLearningPath({ limit: 8 }).catch(() => null));
      const [tree, dashboard, today, reviewItems, reportItems, path] = await Promise.all([getKnowledgeTree(), getDashboardSummary(), getTodayTasks(), getReviewsToday(), reportsPromise, pathPromise]);
      setKnowledgeTree(tree); setSummary(dashboard); setTasks(today); setReviews(reviewItems); setDiagnosticReports(reportItems.reports || []); setLearningPath(path);
      await loadWrongData();
  ...
  }, [loadWrongData, selectedKnowledgeId]);

  useEffect(() => { void refresh(); }, [refresh]);
  useEffect(() => { if (activeTab === "wrong") void loadWrongData().catch((err) => setError(err instanceof Error ? err.message : "??????????")); }, [activeTab, loadWrongData]);
  ...
  async function handleStartWrongRecommendation(recommendation: WrongQuestionRecommendation) {
    setLoading(`wrong-rec-${recommendation.recommendation_id}`); setError(""); setPracticeResult(null); setPracticePdf(null);
    try {
      const tabId = `wrong-rec-${recommendation.knowledge_id}-${Date.now()}`;
      const session = await generatePractice({
        source: "knowledge",
        knowledge_id: recommendation.knowledge_id,
        question_type: recommendation.practice.question_type || recommendation.question_type || undefined,
        question_kind: (recommendation.practice.question_kind as QuestionKind | undefined) || "variant",
        tab_id: tabId,
        limit: recommendation.practice.limit || 5,
      });
      const nextTab: PracticeTabState = { tabId: session.tab_id || tabId, label: `${recommendation.knowledge_name}专项`, source: "knowledge", session, answers: {}, imageAnswers: {}, result: null };
      setPracticeTabs((prev) => [...prev.filter((tab) => tab.tabId !== nextTab.tabId), nextTab]);
      setActivePracticeTabId(nextTab.tabId);
      setPractice(session); setAnswers({}); setImageAnswers({}); setActiveTab("practice");
      setNotice("已按错题推荐生成专项练习。");
    } catch (err) { setError(err instanceof Error ? err.message : "生成推荐练习失败"); }
    finally { setLoading(null); }
  }

  function handleStudyWrongKnowledge(knowledgeId: string) {
    setSelectedKnowledgeId(knowledgeId);
    setActiveTab("knowledge");
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[var(--background)]" translate="no">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-5">
        ...
        {activeTab === "wrong" ? <WrongPanel wrongQuestions={wrongQuestions} summary={wrongSummary} recommendations={wrongRecommendations} filters={wrongFilters} selectedIds={selectedWrongIds} loading={loading} onFiltersChange={setWrongFilters} onToggle={handleToggleWrongSelection} onSelectAll={handleSelectAllWrong} onRetry={(wrongId, retryMode) => void handleWrongRetry(wrongId, retryMode)} onBatchRetry={(retryMode) => void handleBatchWrongRetry(retryMode)} onBatchAction={(action) => void handleBatchWrongAction(action)} onReason={(wrongId, reason) => void handleWrongReason(wrongId, reason)} onAskAI={(questionId) => void handleAskAI("question", questionId, "wrong_question_review")} onStartRecommendation={(recommendation) => void handleStartWrongRecommendation(recommendation)} onStudyKnowledge={handleStudyWrongKnowledge} /> : null}
...
function WrongPanel({ wrongQuestions, summary, recommendations, filters, selectedIds, loading, onFiltersChange, onToggle, onSelectAll, onRetry, onBatchRetry, onBatchAction, onReason, onAskAI, onStartRecommendation, onStudyKnowledge }: { wrongQuestions: WrongQuestion[]; summary: WrongQuestionSummary | null; recommendations: WrongQuestionRecommendationsResponse | null; filters: WrongFilters; selectedIds: string[]; loading: string | null; onFiltersChange: (filters: WrongFilters) => void; onToggle: (wrongId: string) => void; onSelectAll: (checked: boolean) => void; onRetry: (wrongId: string, retryMode: "original" | "variant") => void; onBatchRetry: (retryMode: WrongRetryMode) => void; onBatchAction: (action: "mark_focus" | "unmark_focus" | "add_to_review" | "export_selected") => void; onReason: (wrongId: string, reason: string) => void; onAskAI: (questionId: string) => void; onStartRecommendation: (recommendation: WrongQuestionRecommendation) => void; onStudyKnowledge: (knowledgeId: string) => void }) {
